...
class FormSubmissionSerializer(serializers.ModelSerializer):
    ...
    def get_student_details(self, obj):
        if obj.student:
            ...
            # Banking details are restricted to Director role only (Requirement 12)
            request = self.context.get('request')
            if request and getattr(request.user, 'role', None) == 'director':
                details['account_holder_name'] = obj.student.account_holder_name
                details['bank_name'] = obj.student.bank_name
                details['account_number'] = obj.student.account_number
                details['transit_number'] = obj.student.transit_number
            return details
        return None
